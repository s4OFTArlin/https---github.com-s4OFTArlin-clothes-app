        ...
        <div class="modal-card modal-card-mini-body">
          ...
          <div class="input-group">
            ...
            <div class="bwh-inputs">
              <input type="number" v-model.number="tempBody.bust" class="modal-input small">
              <input type="number" v-model.number="tempBody.waist" class="modal-input small">
              <input type="number" v-model.number="tempBody.hips" class="modal-input small">
            </div>
          </div>
          <div class="modal-actions">
...
import { Plus, ChevronRight, Edit3, Camera, Edit2 } from 'lucide-vue-next'
...
const openBodyEdit = (mode, item = null) => {
  ...
  if (mode === 'add') {
    tempBody.id = Date.now(); tempBody.name = ''; tempBody.height = 165; tempBody.weight = 50; tempBody.bust = 85; tempBody.waist = 60; tempBody.hips = 90
  } else {
...
const saveBodyProfile = () => {
  ...
  if (bodyModalMode.value === 'add') {
    ...
    if (index !== -1) bodyProfiles.value[index] = { ...tempBody, isActive: bodyProfiles.value[index].isActive }
  }
...
.data-card.active { border: 1.5px solid #ff4081; background: #fff0f5; }
...
.add-card { min-width: 150px; background: #fff; border-radius: 16px; border: 2px dashed #ddd; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; }
...
.save-btn { width: 100%; background: #ff4081; color: #fff; border: none; border-radius: 10px; font-weight: 700; padding: 10px; margin-top: 5px; }
</style>